"""
dockg_semantic_analysis.py

DocKG Semantic Corpus Analysis.
...
_STOP = frozenset(
    ...
    this that it as by from all can will not also if its your you we i they
    their which what how when where there has have had each into any about more
    use used using no such these those do does did than then so very just
    should would could may might must get got new one two three first last
    other another some many most every both after before over under through
    between during while since until even though although only already still
    always often never here there again further too few same different again
    him her his she he they them us our my
    """.split()
)
...
class DocKGSemanticAnalyzer:
    """Semantic content analyser for a DocKG knowledge graph.

    :param kg: An open :class:`~doc_kg.kg.DocKG` instance.
    :param console: Rich Console for progress output (created if not given).
    """

    def __init__(self, kg: DocKG, console: Console | None = None) -> None:
        self.kg = kg
...
def _default_report_path(corpus_root: Path) -> str:
    ...
    return str(corpus_root / "analysis" / f"doc_kg_semantic_{stamp}.md")


def _default_json_path() -> str:
    return str(Path.home() / ".claude" / "dockg_semantic_latest.json")


def _print_summary(console: Console, result: dict) -> None:
    lang = result.get("language", {})
    table = Table(title="DocKG Semantic Analysis Summary")
...
def main(
    ...
    """Run semantic DocKG analysis and write Markdown + JSON outputs.
    ...
    db = Path(db_path) if db_path else (root / ".dockg" / "graph.sqlite")
    lancedb = Path(lancedb_path) if lancedb_path else (root / ".dockg" / "lancedb")

    kg = DocKG(corpus_root=root, db_path=db, lancedb_dir=lancedb)
    analyzer = DocKGSemanticAnalyzer(kg, console=console)
