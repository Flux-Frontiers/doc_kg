...
from doc_kg import DocKG, mcp_server
...
async def test_lifespan_closes_kg_on_server_shutdown(tmp_path: Path) -> None:
    # An unbuilt corpus is enough: DocKG.stats() reports zeros with an error
    # field rather than raising, and reaching it is what opens the store.
    kg = DocKG(tmp_path)
    mcp_server._kg = kg
    try:
